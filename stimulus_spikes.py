...
def df_filter_stimulus(cell_index, stim_begin, stim_end, file, waveforms=False):
    ...
    return subset_df.filter((pl.col("times") > stim_begin) & (pl.col("times") <= stim_end))
...
def load_triggered(cell_index, stim_begin, stim_end, trigger_start,trigger_end, logic, file, waveforms=False):
    subset_df = df_filter_stimulus(cell_index, stim_begin, stim_end, file, waveforms)
    subset_df = subset_df.with_columns(pl.col("times").sub(stim_begin).alias("times_relative"))
    ...
    dfs = []
    repeat = 0
    for stimulus, rel_t, sub_t in zip(range(trigger_end.shape[0]-1), rel_trigger[:-1], trigger_sub):

        times = subset_df.filter((pl.col("times_relative") > trigger_start[stimulus]) &
                                 (pl.col("times_relative") <= trigger_end[stimulus+1]))
        times = times.with_columns(trigger=pl.lit(rel_t))
        times = times.with_columns(repeat=pl.lit(repeat))

        times = times.with_columns(pl.col("times_relative").sub(sub_t).alias("times_triggered"))
        if rel_t == logic - 1:
            repeat += 1
