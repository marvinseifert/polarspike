...
class Table:
    def __init__(self, df):
        ...
        options = list(
            filter(
                lambda x: x not in ["stimulus_name", "recording"], df.columns.to_list()
            )
        )

        self.column_select = pn.widgets.Select(
            name="Select Column", options=options, value=options[0]
        )
        ...
        self.lower_bound_input.param.watch(
            lambda event: self.filter_df(
                self.lower_bound_input.value,
                self.upper_bound_input.value,
        ...
        self.react = pn.bind(
            ...
            self.column_select,
            self.stimulus_select,
            self.recording_select,
        )  # Add the recording select value here

        # Panel to display the widgets
        self.panel = pn.Column(
            pn.Row(
                pn.Column(
                    self.column_select, self.lower_bound_input, self.upper_bound_input
                ),
                self.stimulus_select,
                self.recording_select,
            ),  # Add the recording select widget here
            self.tabulator,
            self.react,
        )
    ...
    def update_input_range(self, event):
        ...
        self.filter_df(
            ...
            self.stimulus_select.value,
            self.recording_select.value,
        )  # Update the tabulator after changing the input range
