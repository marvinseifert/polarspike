...
single_ab_df = pd.DataFrame(
    data=OP.govardovskii_animal("Zebrafish").flatten(order="F"),
    columns=["absorption"],
)
cones = np.array(["LWS", "MWS", "SWS2", "SWS1"])
cones = np.repeat(cones, 400)
single_ab_df["cone"] = cones
single_ab_df["wavelength"] = np.tile(np.arange(300, 700, 1), 4)
...
fig = px.line(
    ...
    y="absorption",
    color="cone",
    color_discrete_sequence=cone_colours[::-1],
)
...
results_dict["ON"]["Lasso"]["coefs"] = np.zeros((nr_cells, X.shape[0]))
results_dict["ON"]["Lasso"]["goodness_of_fit"] = np.zeros(nr_cells)
results_dict["ON"]["Linear"]["coefs"] = np.zeros((nr_cells, X.shape[0]))
results_dict["ON"]["Linear"]["goodness_of_fit"] = np.zeros(nr_cells)
results_dict["OFF"]["Lasso"]["coefs"] = np.zeros((nr_cells, X.shape[0]))
results_dict["OFF"]["Lasso"]["goodness_of_fit"] = np.zeros(nr_cells)
results_dict["OFF"]["Linear"]["coefs"] = np.zeros((nr_cells, X.shape[0]))
