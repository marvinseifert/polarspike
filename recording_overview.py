...
def isi_from_file(file_parquet, freq=1, x="times", cmap="Viridis"):
    df = pl.scan_parquet(file_parquet)
    df = df.select(pl.col("cell_index"), pl.col("times"))
    df = df.with_columns(pl.col("times").truediv(freq).alias("times"))
    return plot_isi(df, x, cmap=cmap, cutoff=18/freq)
...
def spike_amplitudes_from_file(file_parquet, freq=1, cmap="viridis"):
    ...
    df = df.with_columns(pl.col("times").truediv(freq).alias("times"))
    waves = df.columns[2:]
    df_melt = df.melt(id_vars=["cell_index", "times"], value_vars=waves, variable_name="waveform", value_name="voltage")
    df_group = df_melt.group_by(["cell_index", "times"])
    df_min = df_group.agg(pl.col("voltage").min())
    df_results = df_min.collect(streaming=True)
    return plot_spike_amplitudes(df_results.sort(["cell_index", "times"], descending=False), cmap)

def plot_spike_counts(df, cmap):

    results_df = (
        df.group_by("cell_index")
        .agg(pl.col("times").count().alias("nr_spikes"))
    )
    ...
    artist = dsshow(results_df, ds.Point('cell_index', 'log_nr_spikes'), norm='linear', cmap=cmap,ax=ax)
    ax.set_aspect('auto')

    ax.set_yticks(np.log10([10**i for i in range(max_spike_count)]))  # Replace with the desired ticks in the original scale
...
def plot_isi(df, x, cmap, cutoff=18.0):
    ...
    artist = dsshow(plot_df, ds.Point(x, 'log_isi'), norm='eq_hist', cmap="viridis", ax=ax)
    ax.set_aspect('auto')

    ax.set_yticks(np.log10([10**i for i in range(max_isi)]))  # Replace with the desired ticks in the original scale
    ax.set_yticklabels([10**i for i in range(max_isi)])

    ax.hlines(np.log10(cutoff), 0, max_x, color='red', linestyle='--', linewidth=2, label="1ms cutoff")

    norm = mcolors.LogNorm(vmin=plot_df['isi'].min(), vmax=plot_df['isi'].max())

    ax.legend()
    sm = ScalarMappable(cmap="viridis", norm=norm)
    sm.set_array([])  # This line is needed to create the ScalarMappable object but the array is not used
...
def plot_spiketrains(df, cmap, height=None, width=None):
    ...
    artist = dsshow(plot_df, ds.Point('times', 'cell_index'),plot_height=height, plot_width=width, norm='eq_hist', cmap=cmap, ax=ax)
    ax.set_aspect('auto')
...
def plot_spike_amplitudes(df, cmap):
    ...
    artist = dsshow(plot_df, ds.Point('times', 'voltage'), plot_height=120, norm='eq_hist', cmap=cmap, ax=ax)
    ax.set_aspect('auto')
